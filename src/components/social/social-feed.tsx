...
export function SocialFeed({ initialQuery = "" }: { initialQuery?: string }) {
  ...
  return <AppShell><div className="mx-auto max-w-5xl"><header><h1 className="text-2xl font-semibold">Comunidade</h1><p className="mt-1 text-xs text-muted">Pessoas, grupos e estudo compartilhado.</p><nav className="mt-5 flex gap-6 border-b border-line" aria-label="Seções da comunidade"><Link href="/comunidade" className="border-b-2 border-accent pb-3 text-sm font-semibold text-accent">Feed</Link><Link href="/comunidade/grupos" className="border-b-2 border-transparent pb-3 text-sm font-semibold text-muted hover:text-foreground">Grupos</Link></nav></header>
    ...
    </div><aside className="hidden xl:block"><MySocialProfileCard /><div className="mt-8"><h2 className="text-sm font-semibold">Pessoas para conhecer</h2><div className="mt-4 space-y-6">{data.profiles.filter((profile) => profile.id !== me.id).map((profile) => <div key={profile.id}><Link href={`/comunidade/${profile.username}`} className="flex items-center gap-3"><UserAvatar profile={profile} /><span className="text-sm font-semibold">{profile.name}<span className="block text-xs font-normal text-muted">@{profile.username}</span></span></Link><p className="my-3 text-xs text-muted">{profile.objective}</p><FollowButton profileId={profile.id} /></div>)}</div></div><p className="mt-8 border-t border-line pt-4 text-xs leading-6 text-muted">Anotações, dúvidas e pequenas descobertas também fazem parte do estudo.</p></aside></div>
  </div></AppShell>;
}
