...
export default function ProfilePage() {
  ...
  const community = useSocial();
  const metrics = calculateMetrics(data);
  const groups = community.data.groups.filter((group) => community.data.groupMembers.some((member) => member.groupId === group.id && member.userId === community.me.id));
  return <AppShell><div className="mx-auto max-w-4xl"><header><p className="text-sm text-accent">Seu percurso de estudo</p><h1 className="mt-2 text-3xl font-semibold">{data.profileName || "Seu perfil"}</h1><p className="mt-3 text-secondary">{data.goal?.title || "Defina seu objetivo para começar."}</p><Link href="/configuracoes" className="mt-3 inline-block text-sm text-accent">Editar nome e objetivo</Link></header>
    ...
    <section className="mt-8"><h2 className="text-xl font-semibold">Grupos</h2><p className="mt-2 text-xs text-muted">Seus grupos na comunidade compartilhada.</p><ul className="mt-3 divide-y divide-line">{groups.map((group) => <li key={group.id} className="py-3"><Link className="text-accent" href={`/comunidade/grupos/${group.slug}`}>{group.name}</Link></li>)}</ul>{!groups.length && <Link className="mt-3 inline-block text-sm text-accent" href="/comunidade">Explorar grupos</Link>}</section>
