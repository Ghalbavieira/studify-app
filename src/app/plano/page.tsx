...
export default function PlanoPage() {
  ...
  const metrics = calculateMetrics(data, new Date(`${start}T12:00:00`));
  ...
  return <AppShell><header className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-sm text-accent">Planejamento semanal</p><h1 className="mt-1 text-3xl font-semibold">Seu plano</h1><p className="mt-2 text-sm text-muted">Organize a sequência, o tempo e o conteúdo de cada dia.</p></div>{data.subjects.length > 0 && <button onClick={() => addBlock(start)} className={`${buttonClass} flex items-center gap-2`}><Plus size={18} />Novo bloco</button>}</header>
    {!data.goal || !data.subjects.length ? <div className="mt-8"><EmptyState title="Comece pelo conteúdo" description="Defina seu objetivo e adicione matérias antes de distribuir seus blocos." href={!data.goal ? "/cadastro" : "/materias"} action={!data.goal ? "Criar objetivo" : "Adicionar matérias"} /></div> : <>
      ...
      <div className="flex flex-wrap justify-between gap-3 border-b border-line py-4 text-sm text-muted"><span>Executado: {timeLabel(metrics.executedSeconds)} · {metrics.questions} questões</span><span>Planejado: {blocks.reduce((sum,b) => sum + b.plannedQuestions,0)} questões</span><Link href="/ia" className="text-accent">Recomendação de ajuste →</Link></div>
      ...
      <div className="mt-5 flex flex-wrap items-center justify-between gap-3"><p className="text-xs text-muted">Arraste para mover e reordenar. No celular, use “Mover para”.</p><button disabled={saving || !plan.can("canUseAdvancedPlanning") || !blocks.some((block) => !block.done)} onClick={() => {
        if (!window.confirm("Distribuir os blocos pendentes pela semana? O tempo total e os concluídos serão preservados.")) return;
